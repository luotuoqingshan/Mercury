...
private:
  ...
  vidType *small_edges;   
  vidType *large_edges;
  eidType *large_vertices;           
  eidType *small_vertices;
  eidType n_small_edges;    
  eidType n_large_edges;       
  //=====
public:
  //=====
  eidType small_edge_begin(vidType v) const { return small_vertices[v]; }
  eidType small_edge_end(vidType v) const { return small_vertices[v+1]; }
  vidType get_small_EdgeDst(eidType e) const { return small_edges[e]; } 
  eidType large_edge_begin(vidType v) const { return large_vertices[v]; }
  eidType large_edge_end(vidType v) const { return large_vertices[v+1]; }
  vidType get_large_EdgeDst(eidType e) const { return large_edges[e]; } 
  
  eidType* small_out_rowptr() { return small_vertices; }
  vidType* small_out_colidx() { return small_edges; }
  eidType* large_out_rowptr() { return large_vertices; }
  vidType* large_out_colidx() { return large_edges; }

  vidType get_small_degree(vidType v) const { return small_vertices[v+1] - small_vertices[v]; }
  vidType get_large_degree(vidType v) const { return large_vertices[v+1] - large_vertices[v]; }


  eidType num_small_edges() const { return n_small_edges; }
  eidType num_large_edges() const { return n_large_edges; }
  void orientation_with_division(int BOUND);
  void sort_graph_with_division();
  //======
  ...
  void dump_graph(std::string const& coo_path, std::string const& csr_path);
